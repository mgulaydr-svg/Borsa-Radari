...
st.set_page_config(page_title="Master Quant V6.6 | Hata Raporlamalı Sürüm", layout="wide")

# --- 1. SABİTLER VE STRATEJİLER ---
# Yahoo'nun yerel ETF'leri bloklama ihtimaline karşı küresel Ons Altın (GC=F) ve Gümüş (SI=F) eklendi
MADEN_EVRENİ = ["GLDTR.IS", "GMSTR.IS", "GC=F", "SI=F"]
HİSSE_EVRENİ = ["KCHOL.IS", "TCELL.IS", "DESA.IS", "CLEBI.IS", "KONTR.IS", "BRSAN.IS", "OTKAR.IS", "AKSEN.IS", "GLRMK.IS", "MPARK.IS", "TURSG.IS", "ASTOR.IS", "ISCTR.IS", "AKBNK.IS", "ALARK.IS", "ARDYZ.IS", "CVKMD.IS", "MIATK.IS", "ORGE.IS", "YEOTK.IS"]
...
def rejim_motorunu_calistir(veriler):
    if 'XU100.IS' not in veriler: return "NÖTR (BİST100 Bekleniyor)", {}, "", {}
        
    xu100 = veriler['XU100.IS']['Close']
    if len(xu100) < 50: return "NÖTR (Yetersiz Veri)", {}, "", {}
    ...
    else: genislik_orani = 0.0

    son_fiyat = xu100.iloc[-1]
    
    if drawdown_20g <= -10.0 or vol_yuzdelik >= 90.0:
        return "🚨 KRİZ MODU", {"Hisse": "%0", "Nakit/PPF": "%70", "Maden": "%30"}, "%0 (Alım Yok)", {"Fiyat": round(son_fiyat,2), "Piyasa Genişliği": round(genislik_orani,1), "Volatilite": round(vol_yuzdelik,1)}
    elif son_fiyat < (ema200.iloc[-1] * 0.98) or ema50.iloc[-1] < ema200.iloc[-1] or genislik_orani < 40.0:
        return "🔴 DÜŞÜŞ (Ayı)", {"Hisse": "%0-15", "Nakit/PPF": "%50", "Maden": "%35"}, "%0.25 (Düşük)", {"Fiyat": round(son_fiyat,2), "Piyasa Genişliği": round(genislik_orani,1), "Volatilite": round(vol_yuzdelik,1)}
    elif (son_fiyat > ema200.iloc[-1] * 1.02) and (ema50.iloc[-1] > ema200.iloc[-1]) and (genislik_orani > 55.0):
        return "🟢 YÜKSELİŞ (Boğa)", {"Hisse": "%70-100", "Nakit/PPF": "%0", "Maden": "%0-10"}, "%0.75 (Yüksek)", {"Fiyat": round(son_fiyat,2), "Piyasa Genişliği": round(genislik_orani,1), "Volatilite": round(vol_yuzdelik,1)}
    else:
        return "🟡 NÖTR (Testere)", {"Hisse": "%30-50", "Nakit/PPF": "%30", "Maden": "%20"}, "%0.50 (Normal)", {"Fiyat": round(son_fiyat,2), "Piyasa Genişliği": round(genislik_orani,1), "Volatilite": round(vol_yuzdelik,1)}
...
def varlik_analizi(ticker, df, varlik_tipi="Hisse"):
    ...
    # Bilanço Zırhı: Yahoo IP Ban Atarsa Çökmesin Diye Try-Except
    ...
    for ad, p in STRATEJILER.items():
        ...
        if son_fiyat < trail or son_fiyat < atr_s: 
            ...
            # Hacim şartını emtialarda (veya verisi olmayanlarda) göz ardı et
            hacim_ok = True
            if 'Volume' in df.columns and df['Volume'].iloc[-1] > 0 and not pd.isna(df['Vol_SMA20'].iloc[-1]):
                hacim_ok = df['Volume'].iloc[-1] >= df['Vol_SMA20'].iloc[-1] * (1 + p['vol']/100)

            tek_ok = (son_fiyat > ema_t) and (p['rsi_min'] <= rsi <= p['rsi_max']) and hacim_ok
            tem_ok = True if varlik_tipi == "Emtia" else ((0 < fk_val <= p['fk']) and (0 < pddd_val <= p['pddd']))
            
            # Eğer Yahoo bilançoyu (FK) vermediyse temeli zorunlu olarak onaylı say (Sistemin durmaması için)
            if varlik_tipi == "Hisse" and fk_str == "-": tem_ok = True 
            
            if tek_ok and tem_ok: 
                sinyal = "🚫 YASAK" if "DÜŞÜŞ" in rejim_adi or "KRİZ" in rejim_adi else "🟢 AL"
...
def listeyi_islet(girdi_metni, varlik_tipi="Hisse"):
    hisseler_raw = [x.strip().upper() for x in girdi_metni.split(",") if x.strip()]
    if not hisseler_raw: return None
    
    hisseler = []
    for h in hisseler_raw:
        if "=" in h or "^" in h: hisseler.append(h) # GC=F gibi global emtialara .IS ekleme
        elif not h.endswith(".IS"): hisseler.append(h + ".IS")
        else: hisseler.append(h)
    ...
    with st.spinner("Analiz motoru çalışıyor..."):
        for h in hisseler:
            if h in ANA_VERI_DEPOSU and not ANA_VERI_DEPOSU[h].empty:
                res = varlik_analizi(h, ANA_VERI_DEPOSU[h], varlik_tipi)
                if res: sonuclar.append(res)
                else: hatalar.append(f"{h} (Veri Yetersiz)")
            else:
                hatalar.append(f"{h} (Yahoo Bağlantı Hatası)")
                
    if hatalar:
        st.warning("Aşağıdaki varlıklar Yahoo Finance'ten çekilemedi:\n" + ", ".join(hatalar))
...
def radar_analizi(ticker, df, strat_name):
    ...
    teknik_ok = (fiyat > son['EMA_Trend']) and (p['rsi_min'] <= son['RSI_14'] <= p['rsi_max']) and hacim_ok
    ...
    if not stop_oldu and teknik_ok:
        try:
            ...
            if info:
                ...
                if fk > 0: fk_str = str(round(fk, 1))
                temel_ok = (0 < fk <= p['fk']) and (0 < pddd <= p['pddd'])
                if fk_str == "-": temel_ok = True 
        except: 
            temel_ok = True # Info bloklanırsa tekniği kabul et
    
    if stop_oldu: durum = "🔴 SAT"
    elif teknik_ok and temel_ok: durum = "🚫 YASAK (Rejim)" if "DÜŞÜŞ" in rejim_adi or "KRİZ" in rejim_adi else "🟢 ONAY"
    elif teknik_ok and not temel_ok: durum = "⚠️ ŞİŞKİN TEMEL"
    else: durum = "⏳ BEKLE"
    
    return {"Hisse": ticker, "Fiyat": round(fiyat,2), "F/K": fk_str, "Sinyal": durum}

# --- 7. ARAYÜZ (KOKPİT) ---
st.title("🏛️ Master Quant Fon Yönetim Sistemi V6.6")
...
col4.metric("BİST100 Volatilite", f"%{makro_metrikler.get('Volatilite', 0)} Yüzdelik")

c_hisse, c_altin, c_nakit = st.columns(3)
c_hisse.info(f"📈 **Hisse Senedi:** {tahsis_plani.get('Hisse', '%0')}")
c_altin.warning(f"🪙 **Maden (Hedge):** {tahsis_plani.get('Maden', '%0')}")
...
with tab_tarama:
    ...
    if ticker_dosyasi is not None:
        ...
        if st.button("🚀 Dev Taramayı Başlat"):
            ...
            if firsatlar: st.dataframe(pd.DataFrame(firsatlar))
            else: st.warning("Bu profilin katı şartlarını sağlayan hisse bulunamadı.")
    ...
    if aranan:
        ...
        with st.spinner("Röntgen çekiliyor..."):
            ...
            if aranan in df_sorgu:
                ...
                if sonuc: st.dataframe(pd.DataFrame([sonuc]))
            else:
                st.error("Veri çekilemedi. Hatalı kod veya Yahoo engeli.")
